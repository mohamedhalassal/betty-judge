...
@router.get("/my-submissions",dependencies=[Depends(verify_access_token)]) #pyright: ignore
def read_submissions(session: SessionDep,current_user: Annotated[User, Depends(get_current_user)], status: Annotated[SubmissionStatus | None, Query()] = None): #pyright: ignore
    statement = select(Submission).where(Submission.user_id == current_user.id) #pyright: ignore
...
@router.post("/submit", status_code=201,dependencies=[Depends(verify_access_token)]) #pyright: ignore
...
@router.get("/my-submissions/{submission_id}",dependencies=[Depends(verify_access_token)]) #pyright: ignore
def read_submission(submission_id: int, session: SessionDep, current_user: Annotated[User, Depends(get_current_user)]): #pyright: ignore
    submission = session.exec(select(Submission).where(Submission.id == submission_id, Submission.user_id == current_user.id)).first()#pyright: ignore
    if not submission:
